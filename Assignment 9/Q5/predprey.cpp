...
{
    if( (count(w, 'f') + count(w, 'c')) >= count(w, 'r') ) {         // foxes or coyotes eat rabbits
        return (new empty(row, col));
...
{
    if( count(w, 'c') >= count(w, 'f') ) {         // coyotes eat foxes
        return (new empty(row, col));
...
/** if there are too many coyotes or coyote is too old then fox dies */
