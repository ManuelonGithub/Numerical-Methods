...
creature* coyote::next(world& w) {
    ...
    if( count(w, 'c') > ((2*look+1)*(2*look+1))/2 + 1) { 
        ...
        return (new coyote(row, col, age + 1));     // fox is 1 year older
    }
}
...
{
    if( count(w, 'f') >= count(w, 'r') ) {         // foxes eat rabbits
        return (new empty(row, col));
    }else if(age >= lifespan) {                    // rabbit is too old
        return (new empty(row, col));
}else if ((count (w, 'c') + count (w, 'f')) >= count (w, 'r')) {
return (new empty(row, col));
    }else {
...
{
    if( count(w, 'f') > ((2*look+1)*(2*look+1))/2 + 1) { 
        return (new empty(row, col));            // too many foxes
    }else if(age >= lifespan) {                  
        return (new empty(row, col));            // fox is too old
}else if( count(w, 'c') > count(w, 'f') + 1){
return (new empty(row, col));
    }else {
...
void ecosys::evolve(void)
{
    ...
    sout << "   cycle   grass rabbits   foxes\n";
    sout << "--------------------------------\n";
    fout << "inital world\n";
    sout << setw(8) << i;
